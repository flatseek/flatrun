...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run a prompt through any FlatRun-supported model (GGUF/SafeTensors/MLX).",
    )
    parser.add_argument(
        "--model",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional system message - prepended when --prompt is used.",
    )
    parser.add_argument(
        "--messages-json",
        type=str,
    ...
    parser.add_argument(
        "--no-chat-template",
        action="store_true",
        help="Skip the chat template; treat --prompt as raw text.",
    )
    parser.add_argument(
        "--cache-mb",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Tokens to generate after the prompt. Keep low - this is slow.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
    ...
    # Sampling flags. Defaults match the Qwen2.5-Coder recommended
    # recipe for code completion (near-greedy, low temperature).
    parser.add_argument(
        "--temperature",
        type=float,
        default=0.11,
    ...
    # Accept either a model directory or a single .gguf file path. The
    # file-path form is convenient when there's no sibling tokenizer
    # directory - we'll wrap the file in a virtual directory later.
    ...
    if model_path.is_file():
        if model_path.suffix.lower() != ".gguf":
            print(
                f"FlatRun only accepts a single .gguf file as a file path; "
                f"got {model_path.name}. For other formats point at the "
                f"directory containing the model.safetensors files.",
                file=sys.stderr,
            )
            return 1
        # Synthesise a one-file directory so the rest of the CLI is
        # happy. We keep the .gguf path around for later (tokenizer
        # build, _build_config_from_gguf).
